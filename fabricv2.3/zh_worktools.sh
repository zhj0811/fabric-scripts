...
#应该在 /etc/hosts 里面添加所有的 域名 和 IP 的映射
    ...
    OSNENDPOINT="orderer.example.com:7053"
...
CC_PACKAGE_ID=fabric-normal_1:388745f8836bcdb7afc1b65e065d6e3085b7f105028f7f20fb86e02d2e8b1ef2
...
FLAGS="-o ${ORDERADDRESS} -n ${CCNAME} -v ${CCVERSION}  -C ${CHANNEL_NAME} --sequence 1 --tls true --cafile ${ORDERER_CA}"
CCFLAGS=$FLAGS
if [ "$INITREQUIRED" = "true" ]; then
  CCFLAGS="$CCFLAGS --init-required"
fi
echo "FLAGS ${FLAGS} "
...
1_SetConfig (){
  ...
  set -x
    ...
    osnadmin channel join -c $CHANNEL_NAME --config-block ./channel-artifacts/${CHANNEL_NAME}.block -o $OSNENDPOINT --ca-file "$ORDERER_CA" --client-cert "$ORDERER_ADMIN_TLS_SIGN_CERT" --client-key "$ORDERER_ADMIN_TLS_PRIVATE_KEY" >&log.txt
  res=$?
...
8_getinstalledpackage () {
  ...
  peer lifecycle chaincode checkcommitreadiness $CCFLAGS --signature-policy "$POLICY" --tls true --cafile $ORDERER_CA --output json
  set +x
}
...
9_approveformyorg () {
  for ch in ${NODES[*]}; do
    setGlobals $ch
    set -x
    peer lifecycle chaincode approveformyorg $CCFLAGS
  ...
  sleep 2
  peer lifecycle chaincode checkcommitreadiness $CCFLAGS --output json
  set +x
}
...
10_commit () { 
  setGlobals 0
  set -x
  peer lifecycle chaincode checkcommitreadiness -o $ORDERADDRESS -n $CCNAME  -C $CHANNEL_NAME --version $CCVERSION --sequence 1 --tls true  --signature-policy "$POLICY"  --cafile $ORDERER_CA --output json
